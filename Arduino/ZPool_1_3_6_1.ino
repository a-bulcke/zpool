 ...
 * @version 1.3.6.1
 * @changelog
 *   1.3.6.1 - Correctif remontees pH/ORP : le report periodique "max interval" de la stack
 *             ESP Zigbee ne se declenche pas si l'attribut n'est jamais touche via
 *             setAnalogInput(). Heartbeat firmware ajoute : remontee forcee toutes les
 *             report_time secondes (60 s) meme sans franchissement de seuil.
 *   1.3.6   - Optimisations :
 *             * Une seule lecture DS18B20 par cycle (T passee en parametre a readPH)
 *             * Conversion DS18B20 non bloquante : lectures ADS1115/pression pendant les 750 ms
 *             * Suppression du setAnalogInput inconditionnel (le seuillage SEUIL_PH/SEUIL_ORP est
 *               desormais reellement effectif)
 *             * Calibration testee EN TETE de boucle : plus de cycle de mesure/remontee parasite
 *               avec la sonde dans la solution tampon (suppression de calibInProgress)
 *             * Debounce NVS sur les sliders Z2M (commit differe 5 s, usure flash reduite)
 *             * readTemp robuste : code erreur verifie, rejet 85.0/hors plage, derniere valeur valide
 *             * Version centralisee (FW_VERSION_*), timeout connexion Zigbee (reboot),
 *               vTaskDelayUntil pour periode de cycle constante, fix uint32_t millis()
 *   1.3.5.2 - Seuils de remontee pH et Orp dans sensors_values_update et pas dans setAnalogInputReporting
 *   1.3.5.1 - inversion broches ADS1115 pour ph et ORP
 *   1.3.5   - Pas de remontee pendant la calibration - timing lecture temperature a 750ms
 *   1.3.3   - Correction thermique par equation de Nernst
 *             mv normalise a T_cal avant application du polynome. Point isopotentiel = pH 7.
 ...
 *   1.3.0   - Ajout calibration pH 10.01 (ep7), equation polynomiale degre 2, filtre de Kalman pH+ORP
 *   1.2.4   - OTA
 */
...
/* ──────────────────────────────────────────────────────────────
 *  Version firmware (source unique)
 * ────────────────────────────────────────────────────────────── */
#define FW_VERSION_STR   "1.3.6.1"
#define FW_VERSION_ZB    136          // setVersion / setHardwareVersion (uint8)
#define FW_VERSION_OTA   0x01030601   // fileVersion OTA (maj.min.patch.build)
...
// Constantes de mesure et remontée zigbee
#define ORP_GAIN            2.0f
#define SEUIL_PH            0.05f
#define SEUIL_ORP           10.0f
#define report_time         60       // heartbeat de remontée pH/ORP en secondes
#define CYCLE_MS            10000    // période de la tâche capteurs
#define DS18B20_CONV_MS     750      // conversion 12 bits DS18B20
#define NVS_COMMIT_DELAY_MS 5000     // debounce écriture NVS des sliders
#define ZB_CONNECT_TIMEOUT  120000UL // timeout connexion réseau au boot (ms)
...
float    getTempChecked();
float    readTemp();
uint16_t readPressure();
float    readPHmV();
float    readORPmV();
float    computePH(float mv, float T);
float    computeORP(float mv_raw);
void     setPH4(bool);
void     setPH7(bool);
void     setPH10(bool);
void     setORP(bool);
...
void     calibration();
float    mVtopH(float mv, float T = 20.0f);   // défaut ici, PAS dans la définition
bool     computePolyCoeffs();
void     commitPendingNVS();
...
/* ──────────────────────────────────────────────────────────────
 *  Écritures NVS différées (debounce sliders Z2M)
 *  La valeur est appliquée en RAM immédiatement (callback),
 *  le commit flash n'a lieu qu'après NVS_COMMIT_DELAY_MS de stabilité.
 * ────────────────────────────────────────────────────────────── */
struct PendingNVS {
    const char* key;
    float       value;
...
PendingNVS pendPH4  = { "ph401",  0.0f, false, 0 };
PendingNVS pendPH7  = { "ph700",  0.0f, false, 0 };
PendingNVS pendPH10 = { "ph1001", 0.0f, false, 0 };
PendingNVS pendORP  = { "orp",    0.0f, false, 0 };

void markPending(PendingNVS &p, float val) {
    p.value      = val;
    p.dirty      = true;
    p.lastChange = millis();
}
...
void commitPendingNVS() {
    PendingNVS* all[4] = { &pendPH4, &pendPH7, &pendPH10, &pendORP };
    for (PendingNVS* p : all) {
        if (p->dirty && (millis() - p->lastChange) > NVS_COMMIT_DELAY_MS) {
            calib.putFloat(p->key, p->value);
            p->dirty = false;
            Serial.printf("NVS commit : %s = %.2f\r\n", p->key, p->value);
        }
    }
}
...
void printCalibSummary() {
    ...
    Serial.println();
    Serial.println(SEP);
    Serial.printf ("       ZPool v%s — PARAMETRES DE CALIBRATION (NVS)\r\n", FW_VERSION_STR);
    Serial.println(SEP);
...
void setup() {
    ...
    Serial.printf("Starting ZPool v%s...\r\n", FW_VERSION_STR);
    ...
    zbTempSensor.setVersion(FW_VERSION_ZB);
    zbTempSensor.setHardwareVersion(FW_VERSION_ZB);
    zbTempSensor.addOTAClient(FW_VERSION_OTA, 0x00000000, 0x0001, 0x4142, 0x1011, 223);
    ...
    /* ── Valeurs des solutions (analog output) ────────────────── */
    ...
    // Connexion au réseau avec timeout (évite un blocage infini si coordinateur absent)
    Serial.print("Connexion au reseau ");
    uint32_t zbT0 = millis();
    while (!Zigbee.connected()) {
        Serial.print(".");
        delay(100);
        if ((millis() - zbT0) > ZB_CONNECT_TIMEOUT) {
            Serial.println("\r\nTimeout connexion Zigbee ! Reboot...");
            delay(500);
            ESP.restart();
        }
    }
    ...
    // Valeurs des solutions (modifiables depuis Z2M)
    ...
    // NB : le report périodique "max interval" de la stack ne se déclenche pas
    // de façon fiable si l'attribut n'est jamais touché via setAnalogInput().
    // Le heartbeat est donc assuré par le FIRMWARE (report_time, voir la tâche
    // capteurs). Cette configuration est conservée à titre défensif uniquement.
    zbAnalogPH.setAnalogInputReporting(0, 60, 0);
    zbAnalogORP.setAnalogInputReporting(0, 60, 0);
}
...
void loop() {
    if (digitalRead(button) == LOW) {
        delay(100);
        uint32_t startTime = millis();   // uint32_t : robuste au wrap de millis()
...
/* ══════════════════════════════════════════════════════════════
 *  CALLBACKS — VALEURS DES SOLUTIONS (analog output)
 *  RAM immédiate + commit NVS différé (debounce, voir commitPendingNVS)
 * ══════════════════════════════════════════════════════════════ */

/**
 * @brief Appelé par Z2M/HA quand l'utilisateur modifie la valeur cible pH4.
 *        Mise à jour RAM + recalcul du polynôme. Commit NVS différé (5 s).
 */
void onPH4ValChange(float val) {
    ...
    if (fabsf(val - ph401.ph) < 0.005f) return;   // pas de changement significatif
    ph401.ph = val;
    markPending(pendPH4, val);
    Serial.printf("Solution pH4 mise a jour : %.2f (NVS differe)\r\n", val);
    computePolyCoeffs();
    kfPH.reset();
}
...
void onPH7ValChange(float val) {
    ...
    if (fabsf(val - ph700.ph) < 0.005f) return;
    ph700.ph = val;
    markPending(pendPH7, val);
    Serial.printf("Solution pH7 mise a jour : %.2f (NVS differe)\r\n", val);
    computePolyCoeffs();
    kfPH.reset();
}
...
void onPH10ValChange(float val) {
    ...
    if (fabsf(val - ph1001.ph) < 0.005f) return;
    ph1001.ph = val;
    markPending(pendPH10, val);
    Serial.printf("Solution pH10 mise a jour : %.2f (NVS differe)\r\n", val);
    computePolyCoeffs();
    kfPH.reset();
}
...
void onORPValChange(float val) {
    ...
    if (fabsf(val - orp256.ph) < 0.5f) return;
    orp256.ph = val;
    markPending(pendORP, val);
    Serial.printf("Solution ORP mise a jour : %.1f mV (NVS differe)\r\n", val);
    kfORP.reset();
}
...
/**
 * @brief Récupère la température APRÈS conversion (ds.request() déjà fait).
 *        Vérifie le code d'erreur et la plausibilité ; en cas d'échec,
 *        retourne la dernière valeur valide (85.0 = power-on reset DS18B20).
 */
float getTempChecked() {
    static float lastGood = 20.0f;
    float t = NAN;
    uint8_t err = ds.getTemp(addr[0], t);
    if (err || isnan(t) || t == 85.0f || t < -20.0f || t > 60.0f) {
        Serial.printf("WARN readTemp err=%u val=%.2f -> garde %.2f degC\r\n", err, t, lastGood);
        return lastGood;
    }
    lastGood = t;
...
/**
 * @brief Lecture bloquante (request + 750 ms + getTemp).
 *        Utilisée uniquement par la calibration ; la tâche capteurs
 *        utilise le schéma non bloquant (request / lectures / getTemp).
 */
float readTemp() {
    ds.request();
    vTaskDelay(pdMS_TO_TICKS(DS18B20_CONV_MS));
    return getTempChecked();
}
...
float readORPmV() {
    ...
    return ADS.toVoltage(v) * 1000.0f / ORP_GAIN;
}

/**
 * @brief Conversion mV → pH filtré. La température est fournie par
 *        l'appelant : une seule lecture DS18B20 par cycle.
 */
float computePH(float mv, float T) {
 ...
 *
 *  NB : l'argument par défaut (T = 20.0f) est déclaré dans le PROTOTYPE
 *  uniquement — jamais dans la définition (fragile avec le préprocesseur .ino).
 * ══════════════════════════════════════════════════════════════ */
 ...
 *  Appelée EN TETE de boucle de la tâche capteurs : aucune mesure
 *  ni remontée Zigbee n'a lieu tant qu'une calibration est demandée.
 * ══════════════════════════════════════════════════════════════ */
void calibration() {
    if (ph401.cal) {
        for (int i = 0; i < 100; i++) { mVmoyen.addValue(readPHmV()); vTaskDelay(pdMS_TO_TICKS(100)); }
    ...
    if (orp256.cal) {
        for (int i = 0; i < 100; i++) { mVmoyen.addValue(readORPmV()); vTaskDelay(pdMS_TO_TICKS(100)); }
 ...
 *
 *  Ordonnancement d'un cycle (période constante CYCLE_MS) :
 *    1. Calibration demandée ?  → calibration() AVANT toute mesure
 *       (plus de remontée parasite avec la sonde dans le tampon)
 *    2. ds.request()            → lance la conversion DS18B20
 *    3. Pendant les 750 ms      → pression + ADS1115 pH/ORP + commits NVS
 *    4. getTempChecked()        → température validée
 *    5. Conversions + remontées Zigbee (seuil OU heartbeat report_time)
 * ══════════════════════════════════════════════════════════════ */
static void sensors_values_update(void *arg) {
    ...
    static float    lastPh = NAN, lastOrp = NAN;
    static uint32_t lastPhReport = 0, lastOrpReport = 0;
    ...
    for (;;) {
        ...
        // 1. Calibration prioritaire : aucune mesure/remontée ce cycle
        if (ph401.cal || ph700.cal || ph1001.cal || orp256.cal) {
            calibration();
            lastWake = xTaskGetTickCount();   // resynchronise la période
            continue;
        }

        // 2. Lancer la conversion DS18B20 (non bloquant)
        ds.request();
        uint32_t t_req = millis();

        // 3. Travailler pendant la conversion (750 ms masquées)
        uint16_t pression = readPressure();
        float    mv_ph    = readPHmV();
        float    mv_orp   = readORPmV();
        commitPendingNVS();                    // écritures flash différées

        while ((millis() - t_req) < DS18B20_CONV_MS)
            vTaskDelay(pdMS_TO_TICKS(10));

        // 4. Température validée (une seule lecture par cycle)
        float temp = getTempChecked();

        // 5. Conversions et remontées
        float ph  = computePH(mv_ph, temp);
        float orp = computeORP(mv_orp);
        ...
        // Remontées pH/ORP : seuil franchi OU heartbeat report_time écoulé.
        // Le heartbeat firmware est indispensable : le report périodique
        // "max interval" de la stack ne part pas si setAnalogInput() n'est
        // jamais appelé (l'entité resterait figée dans Z2M/HA).
        uint32_t now = millis();

        bool phDue = isnan(lastPh)
                  || fabsf(ph - lastPh) >= SEUIL_PH
                  || (now - lastPhReport) >= (report_time * 1000UL);
        if (phDue) {
        ...
        bool orpDue = isnan(lastOrp)
                   || fabsf(orp - lastOrp) >= SEUIL_ORP
                   || (now - lastOrpReport) >= (report_time * 1000UL);
        if (orpDue) {
        ...
        // Période réellement constante (indépendante de la durée des lectures)
        vTaskDelayUntil(&lastWake, pdMS_TO_TICKS(CYCLE_MS));
    }
}
