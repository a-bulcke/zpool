...
void setup() {
    ...
    /* ── Capteurs (analog input) ─────────────────────────────── */
    zbTempSensor.setManufacturerAndModel("ZPool", "ZPool");
    zbTempSensor.setVersion(131);
    zbTempSensor.setHardwareVersion(131);
    zbTempSensor.addOTAClient(0x01030100, 0x00000000, 0x0001, 0x4142, 0x1011, 223);
    zbTempSensor.onOTAStateChange(otaStateChange);
    zbTempSensor.setMinMaxValue(-10, 50);
    zbTempSensor.setTolerance(0.1);
...
void calibration() {
    ...
    if (orp256.cal) {
        for (int i = 0; i < 100; i++) { mVmoyen.addValue(readORPmV()); delay(100); }
        orp256.cal = false; orp256.mv = mVmoyen.getAverage(); mVmoyen.clear();
